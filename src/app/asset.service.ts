...
export class AssetService {
    ...
    public getScenariosJSON(): Observable<any> {
        ...
        return this.http.get<any>('./assets/scenarios.json').pipe(
            map(scenarios => {
                ...
                scenarios.nodes = scenarios.nodes.sort((n1: any, n2: any) => +n1.data.id - +n2.data.id);
    ...
    public getDecodedScenarios(currentNodes: any[], savedScenarioString: string) {
        const savedScenarios = JSON.parse(savedScenarioString);
        currentNodes.forEach((node, index) => {
            const savedNode = savedScenarios.nodes.find((saved: any) => saved.id === node.data.id) || {};
            const matchedBase = this.defaultScenariosJSON.nodes.find((base: any) => base.data.id === node.data.id);
    ...
    public getEncodedScenarios(scenarios: any) {
        /* Save only the attributes that are different from the default */
        const changedNodes = scenarios.nodes.reduce((changedNodes: any[], node: any) => {
            const matchedBase = this.defaultScenariosJSON.nodes.find((base: any) => base.data.id === node.data.id);
            const simpleNode: any = { id: node.data.id };
