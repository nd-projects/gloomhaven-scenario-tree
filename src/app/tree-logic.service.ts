...
export class TreeLogicService {
    ...
    public updateScenario(originalScenarios: any, updatedScenario: any): any {
        const scenarios = cloneDeep(originalScenarios);
        const scenarioIndex = scenarios.nodes.findIndex((scenario: any) => scenario.data.id === updatedScenario.id);
    ...
    private showChildScenarios(scenarios: any, parentId: any) {
        scenarios.edges.filter((edge: any) => (edge.data.source === parentId && (edge.data.type === 'unlocks' || edge.data.type === 'linksto')))
            .forEach((edge: any) => {
                const scenarioIndex = scenarios.nodes.findIndex((scenario: any) => scenario.data.id === edge.data.target);
    ...
    private hideChildScenarios(scenarios: any, parentId: any) {
        scenarios.edges
